...
| DEC-019 | Application chrome uses Radix Themes; document-level brand manifest (EP-201) is separate | Application chrome uses Radix Themes for typography, spacing, radius, colour. Custom token chain (Focus, Client Brand A) retired. Document-level brand variables (Phase 2) remain distinct. | Ad-hoc component styling; Tailwind colour/spacing classes throughout; single shared token layer | Medium — interface implementation, design handoff, Phase 2 brand manifest alignment | UI scaffold prompt 2026-02-23; EP-115 2026-02-25 | 2026-02-25 | Accepted |
